$(document).ready(function(){
    ...
    // thay đổi dạng câu hỏi, số câu hỏi, số đáp án
        ...
        if ( dang_media.includes("Văn bản")){
            ...
            <div class="row">
                <div class="col-md-8 col-sm-12 col-xs-12 form-group">
                    <img id="hinh_anh" style="max-height:600px;max-width:600px; display: block; margin-left: auto;margin-right: auto;" src="/static/custom/images/placeholder.png" alt="chọn hình ảnh" />
    ...
    $("#luu_cau_hoi").on("click", function(){
        ...
        if (so_cau_hoi > 1){
            ...
            formData.append('so_dap_an',so_dap_an);
        ...
        if (dang_cau_hoi.includes("Trắc nhiệm")){
            ...
            var message = check_correct_answer(so_cau_hoi, so_dap_an, dap_an);
            if (message != "Done"){
                Swal.fire({
                    type: 'error',
                    title: 'Lỗi',
            ...
            if (jQuery.inArray("<p><br></p>", nd_dap_an) != -1){
                Swal.fire({
                    type: 'error',
                    title: 'Lỗi',
                    text: 'Chưa nhập nội dung đáp án',
                });
                return false;
            }
            ...
            success : function(msg, status, jqXHR){
                ...
                if(result.status == 'False'){
                    ...
                    Swal.fire({
                        ...
                        $("#hinh_anh").attr("src","/static/custom/images/placeholder.png");
                        $("#media").attr("src","");
                        $("input[type=file]").val('');
                    });
                };
...
function thay_doi_mon(){
    var chu_de_select = $("#chu_de");
    chu_de_select.empty();
    var mon_option = $("#mon option:selected");
    $(".chu_de_"+mon_option.data("id")).each(function(){
        let element = `<option>${$(this).val()}</option>`;
        chu_de_select.append(element);
    });
}
...
function check_correct_answer( so_cau_hoi, so_dap_an , dap_an){
    for(let i=0; i<so_cau_hoi; i++){
        let count = 0;
        for(let j=0; j<so_dap_an; j++){
            if(dap_an[i*so_dap_an+j]==1)count++;
        }
        if(count == 0){
            return `Chưa chọn đáp án đúng cho câu hỏi ${i+1}`
        }else if(count == so_dap_an){
            return `Không có đáp án sai cho câu hỏi ${i+1}`
        }
    }
    return "Done";
}
